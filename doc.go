...
// SDK 默认不注入任何上游版本参数：调用方用 WithBeta 注入版本头，或以 query 形式
// 拼入 Dial url / HTTP baseURL。传输常量对齐参考实现（sub2api openai_ws_*）：
// 16MiB ReadLimit（coder 默认 32KB 过小）、CompressionContextTakeover 压缩、
// WS 层 ping 心跳（30s 间隔 + 2s 超时）、data: SSE 行提取与 [DONE] 终止。
//
